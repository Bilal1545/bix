...
sync_packages() {
    ...
    while read -r line; do
        ...
        if [[ $line =~ \} ]]; then
            ...
            if [[ "$source" == "github" ]]; then
                echo "Installing GitHub package: $pkg"
                dl="$TMPDIR/$asset"
            ...
            if [[ "$source" == "url" ]]; then
                echo "Installing URL package: $pkg"
    ...
    if [[ ${#repo_install[@]} -gt 0 ]]; then
        echo ""
        echo "Running repository install:"
        pm_install "${repo_install[@]}"
    else
        echo "No repository actions required."
    fi
}
